...
from Network import loss_fn, Visual_encoder
import wandb
...
if wan:
    wandb.init(
        ...
        project="Deep-VO-FLowNet",
        name="m1_5e-5_adamW_45",

        # track hyperparameters and run metadata
        config={
        "learning_rate": 5e-5,
        "architecture": "Flownet-VO",
        "dataset": "Custom",
        "epochs": 300,
        }
    )
...
def TrainOperation(
    ...
    for Epochs in range(StartEpoch, NumEpochs):
        ...
        if Epochs<250:
            ...
            Optimizer = torch.optim.AdamW(model.parameters(), lr=1e-6, weight_decay=5e-6)
        ...
        for name in names:
            ...
            for PerEpochCounter in range(NumIterationsPerEpoch):
                Img_train_batch, Img_test_batch, pose_train_batch, pose_test_batch, start_index = GenerateBatch(Basepath, MiniBatchSize, splitPerc, start_index, pose_data)
                # Predict output with forward pass
            ...
            if wan:
                wandb.log({"epoch loss train": average_epoch_loss_train})
    
            average_epoch_loss_val = sum(epoch_loss_val[-NumIterationsPerEpoch:]) / NumIterationsPerEpoch
            if wan:
                wandb.log({"epoch loss val": average_epoch_loss_val})
