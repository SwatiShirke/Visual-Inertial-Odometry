...
from Network import loss_fn, Visual_Inertial_encoder
...
if wan:
    wandb.init(
        ...
        project="Deep-VIO-FLowNet",
        name="m1_5e-5_adamw_45_2",
...
def TrainOperation(
    ...
    pretrained_w = torch.load("./VIO_FlowNet_code/flownets_bn_EPE2.459.pth.tar", map_location='cpu')
    # ipdb.set_trace()
    ...
    for Epochs in range(StartEpoch, NumEpochs):
        ...
        if Epochs<250:
            Optimizer = torch.optim.AdamW(model.parameters(), lr=5e-5, weight_decay=5e-5) # model1 
        else:
            Optimizer = torch.optim.AdamW(model.parameters(), lr=1e-6, weight_decay=5e-5)
        ...
        for name in names:
            ...
            for PerEpochCounter in range(NumIterationsPerEpoch):
                # print("Start")
                Img_train_batch, Img_test_batch, IMU_train_batch , IMU_test_batch, pose_train_batch, pose_test_batch, start_index = GenerateBatch(Basepath, MiniBatchSize, splitPerc, start_index, IMU_data, pose_data)
                # Predict output with forward pass
                # print("End")
            ...
            if wan:
                wandb.log({"avg epoch loss val": average_epoch_loss_val})
...
def main():
    ...
    Parser.add_argument(
        "--CheckPointPath",
        default="Checkpoints_Visual_Inertial_2/",
