...
def GenerateBatch(base_path, MiniBatchSize, start_index, pose_data):
    """
    ...
    while ImageNum < MiniBatchSize and  start_index < 500:
        ...
        img1_path = base_path + os.sep + "test2" + os.sep + str(RandIndex) + ".png"        
        img2_path = base_path + os.sep + "test2" +  os.sep + str(RandIndex + 1) + ".png"
    ...
    Img_test_batch = torch.stack(Img_train_batch) if len(Img_train_batch) !=0 else None
    pose_test_batch =  torch.stack(pose_train_batch) if len(pose_train_batch) !=0 else None

    return Img_test_batch.to(device), pose_test_batch.to(device), start_index
...
def TestOperation(
    ...
    if LatestFile is not None:
        ...
        full_path = "Checkpoints_Visual/" + LatestFile + ".ckpt"
    ...
    for name in names:
        ...
        output_file_path_gt = f"{name}_pose_gt_abs_visual.txt"
        ...
        for i in range(0, 500):
            ...
            Img_test_batch, pose_test_batch, start_index = GenerateBatch(Basepath, MiniBatchSize, start_index, pose_data)

            if Img_test_batch is not None and pose_test_batch is not None:
                model.eval()
                with torch.no_grad():
                    
                    pose_train_predicted = model(Img_test_batch.float()).float()
        ...
        output_file_path = f"{name}_pose_test_predicted_abs_visual.txt"
